...
| Status | **CLOSED — DONE (2026-08-06, Mario's decision).** Everything Mario owns is met and re-verified against HEAD. AC-3′ moves to **T07 §7bis.2** as Ezequiel's item; it is a cost/termination clause beside a correctness theorem, not a correctness gap, and R2.1's answer already volunteers that limitation, so nothing in the response letter waits on it. AC-0…AC-7 met. The minimal `Const`-creation repair is shipped and confirmed in **both** engines (`labeled_dag.py:701–715`, `labeled_dag.cpp:339–375`), and the canonicaliser and `is_isomorphic` do **not** apply it in either engine (`canonical.py:192–201`/`:252–261`/`:388–397`, `canonical.cpp:409–418`, `labeled_dag.py:472–481`). AC-4's UDFS half closed 2026-07-28 from T06 (array `1672959`, 234,865 DAGs, 0 failures). **Open: AC-3′, which is Ezequiel's** (a cost/termination clause beside a correctness theorem, not a correctness gap). The recommendation to close as DONE and move AC-3′ to T07 §7bis.2 — evidence and counter-argument in §7's 2026-08-06 entry — was **accepted by Mario on 2026-08-06**. |
