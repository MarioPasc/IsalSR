...
| **P7** | ✅ **DONE 2026-08-07 (T19). Explored-DAG structural telemetry.** Ezequiel asked for the complexity of the explored DAGs per `(method, arm, problem, seed)`, to test the hypothesis that the `isalsr` arm — and less strongly `hash` — explores structurally harder DAGs. `isalsr.core.complexity` (15 descriptors from one `O(V+E)` sweep, 6.8 µs) + `experiments/models/complexity_telemetry.py`. Sampled at a rule **identical across the three arms of a method**, so the cost cancels in every contrast: Bingo the whole population every 25 generations (its baseline has no per-candidate hook at all), UDFS every 31st candidate. Overhead **measured on probe 1814948**: UDFS 0.001–0.003 %, Bingo 0.69 % on its longest cell (≈0.7 % projected at campaign scale; short converged cells read up to 1.96 % only because the fixed generation-0 sample is amortised over few generations). Identical rule in all three arms, so it cancels in every contrast, and `complexity_time_s` allows exact correction. 25 flat fields on `SearchSpaceResults` + a `complexity.json` sidecar; 9 in `METRIC_EXTRACTORS`, 5 in the CPDT. **Pure Python — `canonical.py` and `src/isalsr/core/native/` are untouched, so the build hash, the equivalence gate and D3 are NOT invalidated.** Probe `slurm/t19_probe` (array 1814948, 24 tasks), verifier `slurm/t19_probe/verify.py`. Write-up: `T19-dag-complexity-telemetry.md` | This falls squarely under §3's dividing line — measured *during* a run, so unrecoverable afterwards. Missing it would mean re-running 12,600 jobs to get a distribution. ⚠ **It moves the certified commit**: `campaign/c2` must be re-cut and Stage C re-run before submission (§4, `slurm/c2_tag_procedure.md`) |
...
|---|---|---|---|
| 2026-08-09 | 🔴 **The deadline defers ~20× more cells than the launcher predicted — the sweep arrays are load-bearing, not a nicety** | Audit of the task logs at 83 % (`grep '^Cells:'`) found **512 deferred cells and still climbing**, against `launcher.sh`'s own simulation of *"5-20 real cells out of 12,600"*. Distribution is concentrated, not uniform: 13 tasks deferred **22 of 25** cells each and one deferred 14 of 25, i.e. the `bundle 25` arrays (UDFS `feynman`, B=27) are the ones the deadline bites, exactly as expected when per-cell cost saturates the 12 h payload and `1.4 × expected chunk` under-provisions. **These tasks exit `COMPLETED`** — deferral is the deadline working as designed, and it is what makes a `TIMEOUT` impossible by construction — so **nothing in `sacct` reveals them**. ⚠ **Consequence for §11.1's 2026-08-07 sweep entry: had the campaign been submitted the way all three runbooks instruct, ~512 cells would simply be absent**, and §5.5 would force dropping ~512 `(method, problem, seed)` triples across all three arms — of order **1,500 cells discarded**, ≈12 % of C2. The estimate that made the sweep look optional was wrong by more than an order of magnitude. **Re-simulate `MIN_TASK_HOURS` / the `1.4×` wall factor against measured per-cell cost before any future campaign**, and treat the sweep as a required stage of the submission, not a recovery step | Claude, log audit |
| 2026-08-09 | ⚠ **One OOM cell at 83 % — `bingo/cherrypicked/vlad_7/isalsr/seed_25`** | Task `1840152_59` (`c2s_bi_cherrypicked`) was OOM-killed after 11 h 23 m: `rc=137`, `Detected 1 oom_kill event`. **The 32 GB decision (§3.3) is not obviously wrong**: `MaxRSS` on the `.batch` step is **1,084,696 K ≈ 1.03 GB** against the 32 GB request, matching Stage D's measured 1.05–1.16 GB peak almost exactly. A steady state of 1 GB with a kill at 32 GB means a **fast allocation spike between accounting samples**, not the gradual `canonical_seen` growth §3.3 sized for — that run reached `total_dags=10,316,090 / unique=5,668,994`, whose `set[int]` is only ≈462 MB at the measured 81.5 B/entry. Note the co-occurring `Canonical string computation exceeded time budget` warning on the same problem. **Blast radius is one cell, and the chunking design contained it exactly as intended**: the task reported `2 ok, 1 failed, 0 deferred`, seeds 26 and 27 completed normally, `Copy-back: 14 file(s) verified`, and both other arms of seed 25 landed. **No action taken: the sweep array `c2w_bi_cherrypicked` (1841259) re-runs the bundle `afterany` and resume skips the two completed cells, so it retries this cell alone at the identical 32 GB** — deliberately not raised, because the sweep must run the same protocol as the main pass. Bingo is not seed-reproducible (the accepted C3 finding), so a retry is a genuine second draw rather than a replay. **If the sweep also OOMs**, §5.5 applies: drop the whole `(bingo, Vlad-7, 25)` triple across all three arms rather than leave the paired design unbalanced | Claude, `sacct` + task log |
...
| 2026-08-06 | Submission rate, not `MaxJobCount`, is the binding constraint | The same submission also hit `Slurm temporarily unable to accept job` / `Resource temporarily unavailable` after **29 of 42** arrays in a tight loop, aborting the launcher before it wrote `job_ids.txt` or submitted the aggregation and leaving 29 untracked arrays. Re-submitting the remaining 13 with a **20 s gap** succeeded with **zero refusals**, so the limit was slurmctld RPC pressure, not a count ceiling. **Measured**: 8,903 array tasks were resident without complaint; largest single array is **420**, so the manual's *per-array* 1,000-job courtesy threshold was never in play. ⚠ Whether 12,600 *simultaneously resident* records clear `MaxJobCount = 15000` (cluster-wide, shared) is **still unmeasured** — the failed tasks left the pending pool as fast as they were created. **Mario's batching proposal (chained ≤1,000-job batches) was assessed and deferred**: a dependency-held job still occupies a record, so chaining reduces the *eligible* set but not the *resident* one and does not address `MaxJobCount`; it would serialise the campaign (~7 × 12 h ≈ 84 h against ~50–63 h) and a topology change invalidates the Stage C certification (§1). **Its real merit is §5.5 pairing robustness** — a problem-batched group holds all arms and both methods, so every completed batch is analysable, whereas today's `(method, arm, suite)` arrays can leave unbalanced pairs. Kept as a designed fallback: the resume logic skips completed cells, so conversion mid-campaign costs only the unfinished ones. Submission is now paced by `slurm/c2_campaign/submit_paced.sh`, which is idempotent. Mario has mailed `soporte@scbi.uma.es` | Claude, sbatch logs |
| 2026-08-06 | ✅ **CORRECTED: k=0 gets a sound key, not an exclusion** | **Mario asked whether the k=0 changes were correctly targeted, on the standing rule that anything hurting IsalSR gets reviewed. They were not.** The row below removed k=0 from *both* sides of ρ; that is an over-correction. Four treatments, all computed on the **same** v5b stream so they are directly comparable — **T1** (all k=0 → one `""` class; C1 and all history) **2.0045 / 2.1102**; **T2** (`b223a08`, in `n_total` only) a ratio over two populations; **T3** (`040beab`, out of both) **1.7872 / 1.8534**; **T4** (sound key, counted normally) **2.0044 / 2.1102** for hash/isalsr. **T4 = T1 to four decimals**, because the unsound merge conflates only ~m classes into one — ~1.5 of 289,000 per run, numerically invisible. So the k=0 collision is a **search-correctness defect, not a ρ defect**, and conflating them cost 12.2 % of ρ for nothing. The fitness cache really did hand `f(x)=x_1` the fitness of `f(x)=x_0` and UDFS really did skip `x_1` — worth fixing — but bare-variable candidates are ordinary redundancy: on a **one-variable** problem every one is literally the same DAG (verified: `X_0` and a repeat both serialise `1\|x0<>`), and Nguyen-3 is one variable and 18.2 % k=0. Fix keeps the soundness repair, drops the haircut: k=0 takes the fixed-order insertion serialisation as key (injective on edgeless DAGs) and then follows the normal path. **No asymmetry** — the hash arm already keys on a fixed-order serialisation for *every* candidate. **🔴 RETRACTION: the claim that C1's Bingo reduction factor was inflated ~13.9 % is WITHDRAWN.** C1 used T1, which is numerically correct here; C1's ρ is unaffected on this axis and only the T16 alphabet correction applies to it. Commit `793a17c`; v5c cancelled mid-flight as superseded, wave **v5d** re-run | Claude, same-stream comparison |
| 2026-08-06 | 🔴 **ρ was a ratio over two populations** *(superseded by the row above — the diagnosis was right, the remedy was not)* | **The k=0 guard's own accounting was wrong, and Stage C v5b passed 19/19 with ρ still inflated.** `n_total` is incremented when a candidate is first *seen*, several steps before the DAG exists to be classified, so the guard excluded k=0 from `n_unique` but left it in `n_total` — making `ρ = n_total / n_unique` a ratio over two different populations. **Measured on the full 1,260-run v5b wave, and the error flatters IsalSR:** bingo/hash **2.0045 → 1.7872 (+12.15 %)**, bingo/isalsr **2.1102 → 1.8534 (+13.86 %)**, udfs/* **unchanged** (cannot reach k=0 — the first independent confirmation of `n_calc_nodes = 5` on real data). **It also corrects the incidence figure**: the Stage D trace put k=0 at 0.0593 %, but that is a 1-in-100 subsample of one *hard* 2-variable problem; over the whole wave it is **12.2 %** of bingo/isalsr's stream and **18.2 %** on Nguyen-3, because easy 1-variable problems keep many tiny individuals alive. **A subsampled trace of one hard problem was not a safe basis for that estimate — the 0.0593 % quoted in `b223a08` is superseded.** `total_dags_explored` now counts structural candidates only; candidates seen stays recoverable as `total_dags_explored + n_nonstructural`. Verified end to end: a run reporting `n_total_dags = 5150` pre-guard now reports **4888 + 262 = 5150** exactly. Accounting moved into `record_nonstructural()` so it is unit-testable rather than two increments that must be kept in step. Commit `040beab`; wave **v5c** re-run on it | Claude, full-wave scan |
...
| A | A5 | Seed set declared (1…30, = C1's set) | ✅ **RE-SIGNED 2026-08-06** | **PASS at 1…30.** `0 ∉ seeds`; campaign set disjoint from Stage C's `{0, 101, 102}`; **14/14** campaign configs declare `n_seeds: 30`, read **through the YAML loader** at `experiment.n_seeds`; seed integers coincide with C1's so §7 needs no subsetting. Enforced continuously as gate **G4** of `slurm/c2_campaign/stage_f_preflight.sh`. ⚠ The gate's own first version was wrong twice — a `*.yaml` glob swept in the four `debug_` configs and the trace config (18, not 14), and it read `n_seeds` from the top level where it does not exist, so all 18 came back `None`. A check that fails for its own reasons is as useless as one that passes for them | `stage_f_preflight.sh` G4 |
| A | A5 | *(superseded)* | 🔴 **REOPENED 2026-08-05** | The 2026-08-03 **PASS** certified `1…20` and no longer describes the campaign (§0.4a superseded). Must be re-signed against **1…30**: `0 ∉ seeds`, Stage C's 0/101/102 still disjoint, all 14 configs declaring `n_seeds: 30`, and the seed integers coinciding with C1's so §7 needs no subsetting. Re-run before the `campaign/c2` tag | `c2_preflight/seed_declaration.md` |
...
| E | E1–E7 | **Re-run on v5d, the owed clean root** | 2026-08-06 | ✅ **7/7 PASS, exit 0, and WITHOUT `--allow-mixed-provenance`** — the obligation the row above carries forward is discharged. Same seven verdicts on 1,260 runs at a single provenance (`793a17c`, `git_dirty: false`, one `build_hash`): E1 103 artefacts / 6 families / 3 arms; E2 3 contrasts × 5 metrics over 16 files; E3 N 3→2 with exactly one dropped; E4 6 emitted, **6 compiled**, zero `nan`; E5 10 CD diagrams at 6 groups; E6 refused exit 2, cell named; E7 refused exit 2, three keys named over 20 forged runs | `c2_stage_e/artefacts/stage_e_certification.{json,md}` |
| C | **ALL** | **Stage C re-certified on v5d (`793a17c`)** | 2026-08-06 | ✅ **VERDICT: GO — 19/19 criteria PASS, 0 SKIPs, 0 blocking failures.** Wave **1,260/1,260 COMPLETED**, single provenance. C1.2 **1260/1260 complete** (61 fields, incl. the new `n_nonstructural`); C1.6 420/420 with ρ≥1 and **100 % ρ>1**; **C1.7 1/420 = 0.24 %** (was 8/420 under the over-corrected accounting); C1.13 0 forbidden labels; C4 206 distinct fingerprints, **0 cross-arm disagreements**. ρ: bingo/isalsr **1.8015**, bingo/hash **1.7501**, udfs/isalsr **1.8539**, both baselines 1.0000 by construction — and `n_nonstructural` is **0.09 %** of the Bingo stream once k=0 is deduplicated rather than passed through, which is what supersedes the 12.2 % figure. **This is the wave the `campaign/c2` tag rests on** | `c2_smoke/c2_preflight/stage_c_certification.{json,md}` |
| C | **ALL** | **Stage C re-certified on v5e (`2ff0050`)** | 2026-08-06 | ✅ **VERDICT: GO — 19/19 PASS, 0 SKIPs, 0 blocking failures.** 1,260/1,260 COMPLETED; provenance single at **`campaign/c2-2-g2ff0050`**, `git_dirty: false`. C1.2 1260/1260; C1.6 420/420 with 100 % ρ>1; C1.7 **1/420 = 0.24 %**; C4 206 fingerprints, 0 cross-arm disagreements. ρ: bingo/isalsr **1.8020**, bingo/hash **1.7492**, udfs/isalsr **1.9507**, both baselines 1.0000 by construction — consistent with v5d (1.8015 / 1.7501), the udfs/isalsr move (1.8539 → 1.9507) being ordinary stream variation. **This wave certifies the range-aware seed guard and is the one the tag must sit on.** ⚠ Note it still runs the SMOKE seed spec; the campaign spec is covered separately by G12 and by the 42-task payload probe | `c2_smoke/c2_preflight/stage_c_certification.{json,md}` |
| — | probe | **Campaign-profile payload probe** | 2026-08-06 | ✅ **42/42 COMPLETED, 42 run logs**, campaign profile at `C2_MAX_TIME=240` into a throwaway root. Three arms × 14, provenance `campaign/c2-2-g2ff0050`, `n_nonstructural` populated (0 on baseline/UDFS, non-zero on the Bingo dedup arms). **This is the check whose absence cost the first submission**: `--test-only` validates a resource request and never runs the worker. Now documented in `slurm/c2_campaign/README.md` §2a as a standing pre-submission step | `results/isalsr/c2_probe/` |
| F | — | Sign-off (Mario) | 2026-08-06 | **Gate: 10/11 PASS, blocking only on G3 (the tag, uncut by design).** `bash slurm/c2_campaign/stage_f_preflight.sh` — G1 SP-1 local==remote clean; G2 SP-2 `engine=cpp`; G4 A5 re-signed at 1…30; G5 D3; G6 Stage C GO; G7 Stage E with no provenance override; G8 campaign root empty; G9 42 arrays / 12,600 tasks; G10 42/42 accepted, **84,381 inodes needed against 92,000 free (7,619 spare)**; G11 P2 satisfied — HEAD differs from the certified commit in **0 executable paths**. **Awaiting Mario's explicit final go: the tag and the 12,600-task submission are SP-0 and are not an agent's to take** | `stage_f_preflight.sh` |
