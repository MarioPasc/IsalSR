...
| C | C1.10 | trajectory non-empty, monotone | 2026-08-04 | 🔴 **FAILED 808/1,260 → FIXED.** 100 % of violations at the final row: `best_r2` carried train R² throughout and test R² on the last row. Both translators corrected; 5 regression tests. §11.1 | job 1758604 |
| C | C1.11 | memory profile → production `--mem` | 2026-08-04 | ⚠ **PASSED on 42 of 1,260 rows — a `JobID` vs `JobIDRaw` join bug, now fixed.** Peak `MaxRSS` over the full 1,260 is **0.67 GB**. **Does not size production — D1.2 at 12 h does** | `stage_c_maxrss.csv` |
...
| C | C4 | Cross-arm data identity | 2026-08-04 | **Core property PASSES 1,260/1,260** (`cross_arm_disagreement = 0`). 🔴 **Escalated: `I.12.1` ≡ `I.34.27` is a genuine D1 duplicate** — Mario's decision. Pagie-1/Keijzer-6 seed-invariance is expected (deterministic grids) and now a declared exemption | `c4_fingerprint_findings.md` |
