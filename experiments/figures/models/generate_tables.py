...
Produces three tables:
  Table 1: Three-axis summary (one row per method × benchmark)
  Table 2: Per-problem R² comparison (full 22-row breakdown)
  Table 3: Computational overhead breakdown (per-method + Bingo k-range)
...
def generate_table1(
    ...
    """Three-axis summary table: one row per (method × benchmark)."""
    rows: list[str] = []
    ...
    tex = (
        ...
        "\\centering\n"
        "\\small\n"
        "\\caption{Three-axis summary of \\IsalSR{} integration. "
        "$\\rho$: reduction factor (IsalSR only). "
        "$R^2$ sig.: problems with significant difference (Holm $\\alpha{=}0.05$). "
        "OH: canonicalization overhead. "
        "$S$: search-only speedup (baseline/IsalSR).}\n"
        "\\label{tab:three_axis}\n"
        "\\begin{tabular}{@{}llc ccc cc@{}}\n"
        "\\toprule\n"
        "Method & Bench. & $n$ & $\\rho$ (mean$\\pm$std) & Redund. "
        "& $R^2$ test (BL/IS) & $R^2$ sig. & OH & $S$ \\\\\n"
        "\\midrule\n"
    )
    tex += "\n".join(rows) + "\n"
    tex += "\\bottomrule\n\\end{tabular}\n\\end{table}\n"
...
# ======================================================================
# Table 3: Computational Overhead Breakdown
# ======================================================================
...
def generate_table3(
    ...
    """Computational overhead breakdown + Bingo k-range sub-table."""
    rows: list[str] = []
    ...
    for method in methods:
        for benchmark in benchmarks:
            ...
            all_canon_ms, all_eval_ms, all_oh, all_speedups = [], [], [], []
            ...
            for prob, d in data.items():
                ...
                if d.get("bl_search") and d.get("is_search"):
                    ...
                    is_s = np.mean(d["is_search"])
                    if is_s > 0:
                        all_speedups.append(bl_s / is_s)

            if not all_canon_ms:
                continue

            canon_mean = np.mean(all_canon_ms)
            eval_mean = np.mean(all_eval_ms) if all_eval_ms else 0.0
            ratio = canon_mean / eval_mean if eval_mean > 0 else float("inf")
            oh_mean = np.mean(all_oh) if all_oh else 0.0
            s_mean = np.mean(all_speedups) if all_speedups else 1.0
            ...
            rows.append(
                f"    {method_label:<6} & {bench_label:<8} "
    ...
    tex = (
        ...
        "\\centering\n"
        "\\small\n"
        "\\caption{Computational overhead breakdown. "
        "$T_{\\mathrm{canon}}$: per-DAG canonicalization cost. "
        "$T_{\\mathrm{eval}}$: per-DAG fitness evaluation cost. "
        "Ratio: $T_{\\mathrm{canon}}/T_{\\mathrm{eval}}$. "
        "$S$: search-only speedup.}\n"
        "\\label{tab:overhead}\n"
        "\\begin{tabular}{@{}ll rr c cc@{}}\n"
        "\\toprule\n"
        "Method & Bench. & $T_{\\mathrm{canon}}$ (ms) & $T_{\\mathrm{eval}}$ (ms) "
        "& Ratio & OH & $S$ \\\\\n"
        "\\midrule\n"
    )
    tex += "\n".join(rows) + "\n"
        ...
        for prob, d in data.items():
            ...
            for mk, oh, pd in zip(
                ...
                if np.isfinite(mk) and np.isfinite(oh) and np.isfinite(pd):
                    k_data.append((int(mk), oh, pd, 0.0))
    ...
    if k_data:
        ...
        tex += (
            "\\begin{tabular}{@{}l rrr@{}}\n"
            "\\toprule\n"
            "Bingo $k$-range & $N$ & OH (\\%) & $T_{\\mathrm{canon}}$ (ms) \\\\\n"
            "\\midrule\n"
        )
        for lo, hi in [(0, 5), (5, 15), (15, 32)]:
            subset = [(oh, pd) for mk, oh, pd, _ in k_data if lo <= mk < hi]
            if subset:
                ohs, pds = zip(*subset)
                tex += (
                    f"    $[{lo}, {hi})$ & {len(subset)} "
    ...
    out = output_dir / "table3_overhead_breakdown.tex"
    out.write_text(tex)
    log.info("Saved %s", out)
...
def main() -> None:
    ...
    generate_table3(results_dir, methods, benchmarks, output_dir)
