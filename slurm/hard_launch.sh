#!/usr/bin/env bash
# =============================================================================
# IsalSR Hard-Tier Experiment Launcher for Picasso HPC
# =============================================================================
#
# Submits SLURM array jobs for the 10 hard benchmarks (5 extended Feynman + 5
# GP-hard classics) for both UDFS and Bingo, baseline + IsalSR variants.
#
# Usage:
#   bash slurm/hard_launch.sh                                  # Submit all 4 groups
#   bash slurm/hard_launch.sh --dry-run                        # Print sbatch only
#   bash slurm/hard_launch.sh --experiment udfs_hard_baseline  # Single group
#   bash slurm/hard_launch.sh --analyze-only                   # Only analysis
#
# This script reuses slurm/workers/models_experiment_slurm.sh as the worker;
# the worker auto-detects the "hard" benchmark via MODELS_BENCHMARK=hard.
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG="${SCRIPT_DIR}/hard_config.yaml"
...
# ---------------------------------------------------------------------------
# Parse hard_config.yaml using Python
# ---------------------------------------------------------------------------
parse_config() {
    "$PYTHON" -c "
...
echo "=============================================="
echo "IsalSR Hard-Tier Experiment Launcher"
echo "=============================================="
echo "Config:     ${CONFIG}"
echo "Repo:       ${REPO_DIR}"
...
submit_analysis() {
    ...
    time_limit=$(get_exp_field "hard_analyze" "time_limit")
    cpus=$(get_exp_field "hard_analyze" "cpus")
    mem_gb=$(get_exp_field "hard_analyze" "mem_gb")
    methods=$(get_exp_field "hard_analyze" "methods")
    benchmarks=$(get_exp_field "hard_analyze" "benchmarks")

    local out_dir="${RESULTS_DIR}/slurm_logs/hard_analyze"
...
if [[ "$ANALYZE_ONLY" == "true" ]]; then
    ...
    if [[ "$SINGLE_EXP" == "hard_analyze" ]]; then
        submit_analysis ""
    else
    ...
    echo "--- Phase 1: UDFS hard experiments ---"
    PHASE1_IDS=()
    for exp in udfs_hard_baseline udfs_hard_isalsr; do
    ...
    echo "--- Phase 2: Bingo hard experiments (after UDFS) ---"
    PHASE2_IDS=()
    ...
    for exp in bingo_hard_baseline bingo_hard_isalsr; do
