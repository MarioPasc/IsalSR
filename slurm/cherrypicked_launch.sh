...
submit_analysis() {
    ...
    time_limit=$(get_exp_field "cherrypicked_analyze" "time_limit")
    cpus=$(get_exp_field "cherrypicked_analyze" "cpus")
    mem_gb=$(get_exp_field "cherrypicked_analyze" "mem_gb")
    methods=$(get_exp_field "cherrypicked_analyze" "methods")
    benchmarks=$(get_exp_field "cherrypicked_analyze" "benchmarks")

    local out_dir="${RESULTS_DIR}/slurm_logs/cherrypicked_analyze"

    echo "[cherrypicked_analyze]" >&2
    ...
    local sbatch_cmd="sbatch \
        --job-name=isalsr_cherrypicked_analyze \
...
# ---------------------------------------------------------------------------
EXPERIMENT_GROUPS=(
    udfs_cherrypicked_baseline
    udfs_cherrypicked_isalsr
    bingo_cherrypicked_baseline
    bingo_cherrypicked_isalsr
)
...
if [[ "$ANALYZE_ONLY" == "true" ]]; then
    ...
    if [[ "$SINGLE_EXP" == "cherrypicked_analyze" ]]; then
        submit_analysis ""
    else
    ...
    for exp in bingo_cherrypicked_baseline bingo_cherrypicked_isalsr; do
