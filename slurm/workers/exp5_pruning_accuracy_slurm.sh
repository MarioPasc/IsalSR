#!/usr/bin/env bash
# SLURM compute worker: Pruning Accuracy (Experiment 5)
# Array job: SLURM_ARRAY_TASK_ID = n_internal value
set -euo pipefail

echo "=== Experiment 5: Pruning Accuracy - SLURM Worker ==="
...
EXP_CFG=$($PYTHON -c "import yaml,json; json.dump(yaml.safe_load(open('${CONFIG}'))['experiments']['exp5_pruning_accuracy'], __import__('sys').stdout)")

SAMPLES=$( echo "$EXP_CFG" | $PYTHON -c "import json,sys; print(json.load(sys.stdin).get('samples_per_node', 500))")
TIMEOUT=$( echo "$EXP_CFG" | $PYTHON -c "import json,sys; print(json.load(sys.stdin).get('timeout', 60))")
SEED=$(    echo "$EXP_CFG" | $PYTHON -c "import json,sys; print(json.load(sys.stdin).get('seed', 42))")

N_INTERNAL=${SLURM_ARRAY_TASK_ID:-1}
OUT_DIR="${RESULTS_DIR}/exp5_pruning_accuracy"
mkdir -p "$OUT_DIR"
...
for NVARS in 1 2 3; do
    ...
    python experiments/scripts/exp5_pruning_accuracy.py \
        --output "${OUT_DIR}/accuracy_v${NVARS}_k${N_INTERNAL}.csv" \
        --num-vars "$NVARS" \
