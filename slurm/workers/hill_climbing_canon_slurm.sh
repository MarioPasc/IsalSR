#!/usr/bin/env bash
# SLURM compute worker: hill_climbing_canon
# Runs 30 independent experiments (seeds 42..71) across all Nguyen benchmarks.
set -euo pipefail

echo "=== hill_climbing_canon: SLURM Worker ==="
...
BENCH_CFG=$(python3 -c "import yaml,json; json.dump(yaml.safe_load(open('${CONFIG}'))['experiments']['hill_climbing_canon'], __import__('sys').stdout)")
