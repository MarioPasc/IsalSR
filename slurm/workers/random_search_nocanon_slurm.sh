#!/usr/bin/env bash
# SLURM compute worker: random_search_nocanon (ARRAY JOB)
# Each array task runs ONE independent experiment.
# Submit with: sbatch --array=1-30 random_search_nocanon_slurm.sh
set -euo pipefail

echo "=== random_search_nocanon: SLURM Array Task ==="
