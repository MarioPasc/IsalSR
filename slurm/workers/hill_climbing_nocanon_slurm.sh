#!/usr/bin/env bash
# SLURM compute worker: hill_climbing_nocanon (ARRAY JOB)
# Each array task runs ONE independent experiment.
# Submit with: sbatch --array=1-30 hill_climbing_nocanon_slurm.sh
set -euo pipefail

echo "=== hill_climbing_nocanon: SLURM Array Task ==="
...
BENCH_CFG=$($PYTHON -c "import yaml,json; json.dump(yaml.safe_load(open('${CONFIG}'))['experiments']['hill_climbing_nocanon'], __import__('sys').stdout)")
