...
\textbf{Result}: 345 valid evaluation points. The Bland--Altman plot
(Figure~\ref{fig:p4}, center) shows the difference $v_{\text{orig}} - v_{\text{canon}}$
is concentrated at zero. The error distribution (right panel) confirms the
vast majority of differences are below machine epsilon ($10^{-8}$).

The non-zero maximum error ($\approx 6.3$) arises in a small number of cases
where floating-point non-associativity in variadic operations (addition of
3+ terms) causes different evaluation order after canonicalization. This is
a known limitation of floating-point arithmetic, not an IsalSR bug.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/P4_evaluation_preservation.png}
\caption{Property~4: Evaluation preservation ($N = 345$ points). Original vs.\
canonical evaluations fall on the identity line (left). The Bland--Altman plot
(center) is flat at $y = 0$. Error distribution (right) shows nearly all
